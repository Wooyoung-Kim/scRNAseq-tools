...
=====================
Code reference for tools/visualization.md rules.
Read visualization.md for rules/output specs FIRST.
"""
...
def calculate_dot_size(n_genes, n_groups):
    ...
    - figsize_per_gene, figsize_per_group: for legacy callers
    """
    ...
    if total_dots <= 50:
        ...
        # ≥150 dots: largest_dot=60, standard formula applies
        return {
            'dot_max': 1.0,
            'smallest_dot': 0,
            'largest_dot': 60,
            'figsize_per_gene': 0.28,
            'figsize_per_group': 0.38
        }
...
def save_dotplot_standard(adata, markers, annotation_col, annotation_order,
    ...
    figsize formula: (n_genes × 0.28 + 1, n_groups × 0.38 + 1.5)
    largest_dot=60, dot_max=1.0, standard_scale='var', use_raw=False
    dpi=100 (PNG), SVG also saved
    ...
    # Standard figsize formula
    width = n_genes * 0.28 + 1
    height = n_groups * 0.38 + 1.5
    ...
    _make_dp().savefig(f'{filepath}.png', dpi=100, bbox_inches='tight')
    plt.close('all')
    _make_dp().savefig(f'{filepath}.svg', bbox_inches='tight')
    plt.close('all')
...
def save_markdown_report(evidence_list, output_path, title="Annotation Report",
    ...
    for i, entry in enumerate(evidence_list, 1):
        ...
        # --- Per-marker unified table (Gene / pct / LFC / PMID / Title / Reasoning) ---
        markers = entry.get('markers', [])
        if markers:
            lines.append("### Marker Evidence")
            lines.append("")
            lines.append("| Gene | pct | LFC | PMID | Title | Reasoning |")
            lines.append("|------|-----|-----|------|-------|-----------|")

            for m in markers:
                gene  = m.get('gene', '')
                pct   = m.get('pct_in', m.get('pct', ''))
                lfc   = m.get('log2fc', m.get('logfoldchanges', m.get('enrichment', '')))
                pmid  = m.get('pmid', '')
                title = m.get('title', '')
                ...
                # Format pct (fraction or percent)
                ...
                # Hyperlink PMID if present
                if pmid:
                    pmid_str = f"[{pmid}](https://pubmed.ncbi.nlm.nih.gov/{pmid}/)"
                else:
                    pmid_str = "—"

                # Truncate long title/reasoning for table readability
                title_str = (title[:80] + "…") if len(title) > 80 else title
                reason_str = reasoning if reasoning else "—"

                lines.append(f"| {gene_str} | {pct_str} | {lfc_str} "
                             f"| {pmid_str} | {title_str} | {reason_str} |")
